...
export default {
  ...
  methods: {
    getCategory () {
      this.$http.get(url.getCateGoryList).then(res => {
        if (res.success) {
          this.category = res.data
          this.getCategorySubByCategoryID(res.data[0].ID)
        } else {
          Toast('服务器错误，数据取得失败')
        }
      }).catch(error => {
    ...
    getCategorySubByCategoryID (categoryId) {
      ...
      this.$http.get(url.getCateGorySubList, {
        params: data
      }).then(res => {
        if (res.success) {
    ...
    getGoodList () {
      this.$http.get(url.getGoodsListByCategorySubID, {
        params: {
